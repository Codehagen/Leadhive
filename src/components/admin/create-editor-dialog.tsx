...
import { createEditor } from "@/app/actions/admin/create-editor";
...
export function CreateEditorDialog() {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    setIsLoading(true);
    const response = await createEditor(values);
    setIsLoading(false);

    if (response.success) {
      toast.success("Editor opprettet", {
        description: "Editoren har blitt lagt til i systemet.",
      });
      setOpen(false);
      form.reset();
      router.refresh();
    } else {
      toast.error("Kunne ikke opprette editor", {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Opprett ny editor</DialogTitle>
          <DialogDescription>
            Fyll ut informasjonen om editoren
          </DialogDescription>
        </DialogHeader>
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Textarea
                      placeholder="Kort beskrivelse av editoren..."
                      {...field}
                    />
                  </FormControl>
                  <FormMessage />
            ...
            <Button type="submit" disabled={isLoading} className="w-full">
              {isLoading ? "Oppretter..." : "Opprett editor"}
            </Button>
          </form>
        </Form>
      </DialogContent>
