...
export function SignUpFormLandscapingAU() {
  const [isLoading, setIsLoading] = useState(false);
  const [agreed, setAgreed] = useState(false);
  const [isSubmitted, setIsSubmitted] = useState(false);
  const [zoneName, setZoneName] = useState<string | null>(null);
  const [landscapingCategoryId, setLandscapingCategoryId] = useState<
    string | null
  >(null);
  ...
  // Fetch landscaping category ID on component mount
  useEffect(() => {
    async function fetchLandscapingCategory() {
      try {
        const categoriesResult = await getCategories();
        if (categoriesResult.success) {
          const landscapingCategory = categoriesResult.data.find(
            (category) => category.label === "Landscaping"
          );
          if (landscapingCategory) {
            setLandscapingCategoryId(landscapingCategory.value);
          }
        }
      } catch (error) {
        console.error("Error fetching landscaping category:", error);
      }
    }
    fetchLandscapingCategory();
  }, []);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!landscapingCategoryId) {
        toast.error("Unable to process landscaping requests at this time");
        return;
      }
      ...
      const response = await createContactRequest({
        ...
        categoryIds: [landscapingCategoryId],
        country: "AU",
      });
      ...
      if (response.success) {
        setIsSubmitted(true);
        toast.success(
          "Thank you! We'll connect you with local landscapers soon."
        );
        triggerConfetti();
      } else {
        toast.error(
      ...
      <div className="space-y-6 text-center">
        <div className="p-6 rounded-lg border bg-background/60 backdrop-blur-xl">
          ...
          <p className="text-muted-foreground mb-4">
            We'll connect you with top local landscapers within 24 hours.
          </p>
    ...
    <div className="space-y-6">
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        >
          ...
          Get matched with landscapers
        </Button>
      </form>

      <p className="text-center text-sm text-muted-foreground">
        Connect with top landscaping professionals in your area
      </p>
    </div>
  );
}
