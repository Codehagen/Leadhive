...
export default async function LandscapingBlogPost({
  params,
}: {
  params: {
    slug: string;
  ...
  if (!data || !data.categories.includes("landscaping")) {
    notFound();
  }
  ...
  const category = BLOG_CATEGORIES.find(
    (category) => category.slug === "landscaping"
  )!;
      ...
      <MaxWidthWrapper>
        <div className="flex max-w-screen-sm flex-col space-y-4 pt-16">
          <div className="flex items-center space-x-4">
            <Link
              href={`/${params.locale}/landscaping/blog`}
              ...
              <div className="flex flex-col space-y-4 py-5">
                ...
                <ul className="flex flex-col space-y-4">
                  {relatedArticles.map((post) => (
                    <li key={post.slug}>
                      <Link
                        href={`/${params.locale}/landscaping/blog/${post.slug}`}
